...
Page({
  ...
  form1Submit: function (e) {
    var that = this;
    var clientid=12212,token=121212;
      ...
      wx.login({
        success: function (res) {
          if (res.code) {
            //发起网络请求
            wx.request({
              url: 'http://47.94.7.71/goodskill/get_newUser.do?jsoncallback=?',
              ...
              success: function (res) {
                ...
                if (data.flag == 1) {
                  ...
                  try {
                    wx.setStorageSync('GoodJob_user', data.user)
                    console.log("本地用户数据写入成功!")
                  } catch (e) {
                  }
                  // wx.setStorage({
                  //   key: '$user',
...
//注册
      ...
      wx.login({
        success: function (res) {
          if (res.code) {
            //发起网络请求
            wx.request({
               url: 'http://47.94.7.71/goodskill/add_newUser.do?jsoncallback=?',
              //url: 'http://192.168.0.146:8080/framework/add_newUser.do?jsoncallback=?',
              data: {
                clientid: clientid,
                token: token,
                phone: that.data.phonenumber,
  ...
  //   // }
  //   if (!this.data.addFlag1 && !this.data.addFlag2) {
  //     console.log("success!")
  //     wx: wx.request({
  //       url: 'http://localhost:8080/framework/get_newUser.do?jsoncallback=?',
  //       data: {

  //         // user_id: 7,
  //         // area_code: "_" + this.data.region[2],
  //         // community_name: e.detail.value.community,
  //         // pre_start_date: this.data.date,
  //         // jobtype_id: this.data.objectJobtypes[this.data.jobtypeIndex].id,
  //         // skills_id: skills_id,
  //         // decoration_type: this.data.radioIndex + 1,
  //         // area: e.detail.value.area,
  //         // unit: this.data.unitIndex + 1,
  //         // salary: e.detail.value.salary,
  //         // urgent_status: this.data.urgent,
